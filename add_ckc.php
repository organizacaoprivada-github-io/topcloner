...
//$mid = mysqli_real_escape_string($db, trim($_POST['mid']));
...
//$keywords = mysqli_real_escape_string($db, trim($_POST['keywords']));
    ...
    if (!($my_blogger_id)) {
        ...
        if (isset($title) && strlen($title) < 3) {
            ...
            if (in_array($url, $banned_urls)) {
                echo "You cannot use that title! It is saved for system files!";
            } else {
                ...
                $command = "INSERT INTO cms (top, mid, footer, title, description, keywords, url, ckc, date) VALUES ('$top', '$mid','$footer','$title', '$description', '$keywords', '$url', 1, now());";
